...
export const reportActionSchema = z.discriminatedUnion('status', [
  ...
  z.strictObject({ status: z.literal('CONFIRMED_FIRE'), ...reportActionFields, confirmed: z.strictObject({
    evidence: z.strictObject({ findings: z.literal('VISIBLE_FIRE'), source: z.string().trim().min(3).max(300).refine(v => v !== reviewerAssessmentSource, 'Actual field observation source is required'), observedAt: timeSchema, latitude: latitudeSchema, longitude: longitudeSchema }),
