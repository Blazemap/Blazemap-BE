...
const verificationInputSchema = z.strictObject({ outcome: z.enum(['CONFIRMED_FIRE', 'NOT_FIRE', 'INCONCLUSIVE']), decisionNote: reasonSchema, observationId: idSchema, version: z.number().int().positive(), relatedReportIds: z.array(idSchema).max(100).refine(ids => new Set(ids).size === ids.length, 'Select each related report once').default([]), ...boundaryObservationFields })
