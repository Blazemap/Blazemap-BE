...
export const relatedReportIdsSchema = z.array(idSchema).max(100).refine(ids => new Set(ids).size === ids.length, 'Select each related report once').default([]);
const verificationInputSchema = z.strictObject({ outcome: z.enum(['CONFIRMED_FIRE', 'NOT_FIRE', 'INCONCLUSIVE']), decisionNote: reasonSchema, observationId: idSchema, version: z.number().int().positive(), relatedReportIds: relatedReportIdsSchema, ...boundaryObservationFields })
...
export const perimeterPatchSchema = z.strictObject({ reporterMessage: reasonSchema, version: z.number().int().positive(), relatedReportIds: relatedReportIdsSchema, perimeter: polygonSchema, perimeterObservedAt: timeSchema, perimeterSource: z.string().trim().min(3).max(300), reason: reasonSchema, authorityReference: z.string().trim().min(3).max(500) });
