...
const field = z.object({ id, findings: z.enum(fieldFindings), source: z.string().min(1).max(300), provenance: z.enum(['FIELD_OBSERVATION', 'REVIEWED_CITIZEN_ESTIMATE', 'OPERATOR_ASSESSMENT']).default('FIELD_OBSERVATION'), sourceReportId: id.nullable().default(null), observedAt: z.date(), ...point });
const caseSchema = z.object({ id, contextRevision: z.number().int().positive(), verificationStatus: z.enum(verificationStatuses), reports: z.array(report), fieldUpdates: z.array(field) });
const weatherSchema = z.object({ id, provider: z.literal('GOOGLE_WEATHER'), issuedAt: z.null(), attribution: z.string(), validAt: z.date(), fetchedAt: z.date(), temperature: z.number().finite().nullable(), humidity: z.number().min(0).max(100).nullable(), windSpeed: z.number().finite().nonnegative().nullable(), windFromDegrees: z.number().min(0).lt(360).nullable() });
...
export function buildAnalysisContext(caseValue: unknown, forecastValue: unknown, spatialValues: unknown[], operationalValues: unknown[], windContext: ReturnType<typeof buildWindContext> | null = null) {
  ...
  const observations = [
    ...
    ...c.fieldUpdates.map(f => ({ id: f.id, kind: f.provenance === 'OPERATOR_ASSESSMENT' || f.source === reviewerAssessmentSource ? 'REVIEWER_ASSESSMENT' : f.provenance === 'REVIEWED_CITIZEN_ESTIMATE' ? 'REVIEWED_CITIZEN_ESTIMATE' : 'FIELD_UPDATE', observedAt: f.observedAt.toISOString(), description: facts({ findings: f.findings, verificationBasis: f.provenance, sourceReportId: f.sourceReportId, independentFieldObservation: f.provenance === 'FIELD_OBSERVATION' }), latitude: coordinate(f.latitude), longitude: coordinate(f.longitude) })),
  ];
  ...
  return {
    ...
    weather: forecast ? { id: forecast.id, provider: forecast.provider, attribution: forecast.attribution, basis: windContext?.basis ?? 'UNAVAILABLE', provenance: windContext?.provenance ?? null, timestamps: windContext?.timestamps ?? null, stale: windContext?.stale ?? false, issuedAt: null, validAt: forecast.validAt.toISOString(), fetchedAt: forecast.fetchedAt.toISOString(), temperature: forecast.temperature, humidity: forecast.humidity, windSpeed: forecast.windSpeed, windSpeedUnit: 'km/h', windFromDegrees: from, windToDegrees: from == null ? null : (from + 180) % 360, directionPrecision: 'DEGREES', measurementType: 'CURRENT_CONDITIONS' } : null,
