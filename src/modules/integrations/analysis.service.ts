...
import { assertCaseOpen } from '../admin/rules.js';
...
async function runAnalysis(actor: Actor | null, id: string, expectedRevision?: number, client: PrismaClient = db()) {
  ...
  const prepared = await client.$transaction(async tx => {
    ...
    const c = await tx.trCase.findUniqueOrThrow({ where: { id }, select: {
      ...
      fieldUpdates: { select: { id: true, findings: true, source: true, provenance: true, sourceReportId: true, observedAt: true, latitude: true, longitude: true }, take: 100, orderBy: { observedAt: 'desc' } },
    } });
    assertCaseOpen(c);
    if (!actor) {
    ...
    const now = new Date();
    const wind = await loadWindContext(tx, c, now);
  ...
  try {
    ...
    return await client.$transaction(async tx => {
      ...
      const current = c.contextRevision === prepared.context.contextRevision && c.handlingStatus !== 'CLOSED';
