...
import { reportDto, reportInclude } from './reports.service.js';
...
export async function listCitizenFeed(actor: Actor, query: unknown, client: PrismaClient = db()) {
  ...
  const [reports, reportTotal, publications, publicationTotal] = await client.$transaction([
    client.trReport.findMany({ where: ownWhere, include: reportInclude, orderBy: [{ createdAt: 'desc' }, { id: 'desc' }], take }),
    client.trReport.count({ where: ownWhere }),
    client.trPublicInformation.findMany({ where: publicationWhere, select: publicationSelect(now), orderBy: [{ publishedAt: 'desc' }, { id: 'desc' }], take }),
    client.trPublicInformation.count({ where: publicationWhere }),
  ]);
  const merged = [
    ...reports.map(report => ({ kind: 'OWN_REPORT' as const, occurredAt: report.createdAt, ...reportDto(report) })),
