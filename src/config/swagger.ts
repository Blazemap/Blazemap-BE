...
const schemas: Record<string, Schema> = {
  ...
  ReportActionInput: schema(validation.reportActionSchema, 'Atomically records one owner-visible history entry, attaches up to five optional owned READY evidence images, and applies the selected report status. CONFIRMED_FIRE requires an authorized reviewer description of at least five characters, explicit VISIBLE_FIRE evidence metadata and a valid closed perimeter; photos remain optional. It creates or reuses an unconfirmed linked case, records the field evidence, derives authority from the current ADMIN role, verifies the case, and saves the operator-mapped perimeter in the same transaction. Already-confirmed cases must use the versioned audited Revise boundary flow. Uploads remain unattached READY staging if any transaction step fails. Retrying the same idempotency key returns the original entry; changed content returns 409.'),
  ...
  VerificationInput: schema(validation.verificationSchema, 'version must match the current case version. fieldUpdateId must belong to this case. CONFIRMED_FIRE requires VISIBLE_FIRE and a valid closed perimeter with observation time and source; the decision and boundary save atomically. NOT_FIRE requires NO_INDICATION. reporterMessage is owner-visible; reason and authorityReference remain private. INCONCLUSIVE preserves verification status. Published case claims must be withdrawn before correcting a prior decision.'),
  ...
  AssignmentInput: schema(validation.assignmentSchema, 'Case must not be CLOSED or NOT_FIRE. Team must be active, non-sample, with an AVAILABLE operational update no older than 24 hours. reason is audited; idempotencyKey retries return the original assignment and changed content returns 409.'),
  AssignmentPatchInput: schema(validation.assignmentPatchSchema, 'version must match. COMPLETED and CANCELLED assignments are immutable. reason is audited.'),
  ...
  CaseForecastRegionInput: schema(validation.caseRegionSchema, 'Operator-selected mapping only. regionId is required but may be null to remove a mapping. A non-null region must be a verified administrative level IV region with a stored BMKG ADM4 code. Coordinates are never used to infer the region.'),
  ...
  Notification: object({ id: string, reportId: string, type: string, title: string, message: string, createdAt: dateTime, readAt: nullable(dateTime) }, ['id', 'reportId', 'type', 'title', 'message', 'createdAt', 'readAt']),
...
operation('get', '/notifications', 'List own notifications', { list: true, query: query(notificationQuery), data: ref('Notification'), meta: object({ pageSize: { type: 'integer', minimum: 1, maximum: 10 }, unreadCount: { type: 'integer', minimum: 0 }, nextCursor: nullable(string) }, ['pageSize', 'unreadCount', 'nextCursor']), description: 'Returns only the current report owner’s persistent notifications, newest first, with opaque owner-validated cursor pagination.' });
...
operation('get', '/admin/warnings/options', 'List eligible warning operational references', { data: array({ type: 'object' }), description: 'Up to 300 verified access/designated-location candidates; returns only non-sample, current timestamped operational observations. Empty results do not prevent warning publication.' });
operation('post', '/admin/warnings', 'Save warning draft', { body: 'WarningSaveInput', status: 201, capability: 'canPublishInformation', limit: 30 });
operation('patch', '/admin/warnings/{id}', 'Edit warning draft or create replacement draft', { body: 'WarningSaveInput', capability: 'canPublishInformation', limit: 30 });
operation('post', '/admin/warnings/{id}/action', 'Publish or withdraw a reviewed warning', { body: 'WarningActionInput', capability: 'canPublishInformation', limit: 30 });
...
operation('get', '/admin/regions', 'List verified operator regions', { query: query(z.object({ search: z.string().trim().max(200).optional(), bmkgMapped: z.enum(['true', 'false']).optional() })), data: array({ type: 'object' }), description: 'Returns up to 100 verified regions for operator selection. bmkgMapped=true restricts to administrative level IV regions with stored BMKG mappings. Raw provider mapping codes are not returned.' });
...
operation('patch', '/admin/cases/{id}/forecast-region', 'Select a verified BMKG forecast region', { body: 'CaseForecastRegionInput', description: 'Versioned and audited operator action. It changes only the internal case region, invalidates the current analysis context, and never publishes, confirms, warns, orders or infers a region from coordinates.' });
...
operation('post', '/admin/reports/{id}/action', 'Record one atomic report action', { body: 'ReportActionInput', status: 201, limit: 30, description: 'The simplified report-review path. Status, description, attachments, and—when confirmed—field evidence, verification, and perimeter commit or roll back together. Does not publish News.' });
...
operation('get', '/admin/users', 'List users', { list: true, query: query(validation.paginationSchema.pick({ page: true, pageSize: true, search: true }).extend({ role: z.enum(validation.roles).optional(), active: z.enum(['true', 'false']).optional(), emailVerified: z.enum(['true', 'false']).optional() })), description: 'Returns a paginated, searchable and filterable administrative projection of account identity, role, activation, verification and effective capabilities. Sessions, credentials, tokens and password material are never returned.' });
operation('get', '/admin/users/{id}', 'Get user administration detail', { description: 'Returns only the safe administrative projection. Sessions, credentials, tokens and password material are never returned.' });
...
operation('get', '/admin/operations', 'Get operational overview', { description: 'Returns teams (up to 200), equipment (300), access/water features (300), operational updates (1,000), assignments (200), freshness, sample markers, authority flags and derived available counts. Sample records and unverified map features never count as available or authoritative.' });
...
operation('post', '/admin/outcomes/publish', 'Publish a completed reviewed outcome to News', { body: 'OutcomePublicationInput', status: 201, capability: 'canPublishInformation', limit: 30, description: 'Concise explicit publication path for CLOSED confirmed cases and declined reports. Confirmation publishes a frozen reviewed perimeter; decline publishes no location. Active confirmed cases stay in Map/Feed and are rejected here. Idempotent per administrator and key.' });
