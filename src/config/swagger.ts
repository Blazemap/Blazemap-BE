...
const schemas: Record<string, Schema> = {
  ...
  CasePatchInput: schema(validation.casePatchSchema, 'Two exclusive variants: existing priority/handlingStatus, or perimeter + perimeterObservedAt + perimeterSource + authorityReference and optional relatedReportIds. Both require reason and matching version. Related reports must be unique, unlinked, reviewed incident estimates and are linked atomically with the boundary revision. Perimeter requires an email-verified active ADMIN with canConfirmIncidents and CONFIRMED_FIRE, at most 1000 total positions, closed rings, at least 3 distinct vertices, no repeats except closure, no self-intersections, nonzero area and contained disjoint holes. Longitude span >=180 degrees is unsupported. Atomically increments version/contextRevision/perimeterRevision, clears latestAnalysisId and audits; never publishes. RESPONDING requires CONFIRMED_FIRE; CLOSED requires all assignments completed or cancelled.'),
