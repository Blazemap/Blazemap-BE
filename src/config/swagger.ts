...
const schemas: Record<string, Schema> = {
  ...
  CasePatchInput: schema(validation.casePatchSchema, 'At least one of priority or handlingStatus is required. version must match the current case version. RESPONDING requires CONFIRMED_FIRE; CLOSED requires all assignments completed or cancelled.'),
...
operation('get', '/admin/cases/{id}', 'Get case detail and evidence', { description: 'Includes reports (100), hotspots (300), field updates (100), verifications (100), analyses (20), assignments (100), timeline (200), weather (24), and spatial context (100), with each collection bounded independently.' });
operation('patch', '/admin/cases/{id}', 'Update case priority or handling', { body: 'CasePatchInput' });
...
operation('get', '/admin/reports', 'List all reports', { list: true, query: query(reportQuery) });
operation('get', '/admin/reports/{id}', 'Get any report and its updates');
